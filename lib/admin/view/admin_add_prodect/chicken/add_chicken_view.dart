...
import '../../../view_model/add_chicken/cubit.dart';
import '../../../view_model/add_chicken/states.dart';
import '../../home_admin/home_admin.dart';

class AddChickenView extends StatelessWidget {
  const AddChickenView({Key? key}) : super(key: key);
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
        create: (context) => AddChickenCubit(),
        child: BlocConsumer<AddChickenCubit, AddChickenStates>(
          listener: (context, state) {
            if (state is AddChickenSendDataSuccessState) {
          ...
          builder: (context, state) {
            if (state is AddChickenLoadingState) {
              return const Center(
                  ...
                  child: Column(
                    children: [
                      Container(
                        ...
                        child: context.read<AddChickenCubit>().selectImage ==
                                false
                            ? IconButton(
                                onPressed: () {
                                  context
                                      .read<AddChickenCubit>()
                                      .getImagePicker();
                                },
                                icon: Icon(
                                  Icons.add,
                            ...
                            : Image.file(
                                fit: BoxFit.fill,
                                File(
                                  context.read<AddChickenCubit>().file!.path,
                                ),
                              ),
                      ),
                      SizedBox(
